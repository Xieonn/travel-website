...
<style>
    ...
    /* Tambahan style untuk input yang dikunci (readonly) */
    ...
    <div class="card">
        ...
        <form method="POST" action="{{ route('admin.destinasi.store') }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" id="name" name="name" class="form-input" value="{{ old('name') }}" placeholder="Cth: Gunung Kerinci">
            ...
            {{-- Kolom Input Teks untuk Nama Wilayah --}}
            <div class="form-group">
                <label for="location" class="form-label">Alamat / Lokasi Destinasi</label>
                <input type="text" id="location" name="location" class="form-input" value="{{ old('location') }}" placeholder="Cth: Kabupaten Kerinci, Jambi">
                @error('location') <span class="form-error">{{ $message }}</span> @enderror
            </div>

            {{-- Area Peta dan Koordinat Visual --}}
            <div class="form-group" style="padding: 1.5rem; background: #f8fafc; border-radius: 12px; border: 1px solid #e2e8f0;">
                ...
                <p style="font-size: 0.85rem; color: #64748b; margin-top: -4px; margin-bottom: 12px;">Klik area pada peta untuk merekam koordinat destinasi secara otomatis.</p>
                
                {{-- Kotak Peta --}}
                <div id="map"></div>
                
                {{-- Input Koordinat yang dikunci (Readonly) --}}
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-top: 1rem;">
                    <div>
                        <label for="latitude" class="form-label" style="font-size: 0.8rem;">Latitude (Lintang)</label>
                        <input type="text" id="latitude" name="latitude" class="form-input" value="{{ old('latitude') }}" readonly placeholder="Menunggu klik peta...">
                    </div>
                    <div>
                        <label for="longitude" class="form-label" style="font-size: 0.8rem;">Longitude (Bujur)</label>
                        <input type="text" id="longitude" name="longitude" class="form-input" value="{{ old('longitude') }}" readonly placeholder="Menunggu klik peta...">
                    </div>
                </div>
                
                @error('latitude') <span class="form-error" style="margin-top: 0.5rem;">Peringatan: Silakan tentukan titik di peta terlebih dahulu!</span> @enderror
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Fokus awal peta (Koordinat Jambi)
        ...
        // Jika form gagal divalidasi, pertahankan titik marker lama
        ...
        map.on('click', function(e) {
            ...
            // Isi kotak input yang tadinya kosong dengan angka
            latInput.value = lat;
            lngInput.value = lng;
        });
    });
